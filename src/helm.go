...
func (h *Helm) AddRepo() (out []byte, err error) {
    username := " --username " + viper.GetString("helm_username")
    pwd := " --password " + viper.GetString("helm_password")
...
func (h *Helm) Install(name string) (xapp Xapp, err error) {
    ...
    rname := viper.GetString("helm.repo-name")
    ns := getHostArgs() + getNamespaceArgs()
...
func (h *Helm) Status(name string) (xapp Xapp, err error) {
    ...
    out, err := h.Run(strings.Join([]string{"status ", name, getHostArgs()}, ""))
    if err != nil {
...
func (h *Helm) Delete(name string) (xapp Xapp, err error) {
    ...
    _, err = h.Run(strings.Join([]string{"del --purge ", name, getHostArgs()}, ""))
    return xapp, err
}
...
func (h *Helm) GetNames(out string) (names []string, err error) {
    ...
    for _, v := range result {
        ...
        if strings.Contains(xappName, "xapp-manager") == false {
...
func getHostArgs() (string) {
    ...
    return fmt.Sprintf(" --host %s.%s:%s", service, namespace , port)
}
